...
# Build the grid
x = np.zeros(m+2,dtype='d')      # process' slice of spatial points
u = np.zeros(m+2,dtype='d')      # process' slice of solution
un= np.zeros(m+2,dtype='d')      # process' slice of NEW solution
xf= np.zeros(m*p, dtype='d')     # global spatial points
uf= np.zeros(m*p, dtype='d')     # global solution points

x = np.linspace(x_min, x_max, m+2)  # TODO: make this work for > 1 process
...
for j in range(1,nt):
    ...
    #     un[i] = u[i] + K*(u[i-1] - 2.0*u[i] + u[i+1] )
